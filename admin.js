...
async function loadAdminData() {
  const data = await window.RVUFirebase.loadCampusData({ superAdmin: true });
  state.data = {
  ...
  return `
    <main class="login">
      <section class="login-card">
        ...
        <div class="form">
          <div class="form-grid">
            ${input("authEmail", "RVU email", state.authEmail, "email")}
            ${input("authPassword", "Password", state.authPassword, "password")}
          </div>
          <div class="actions">
            <button class="btn gold" data-action="login-password">${state.authMode === "signup" ? "Create account" : "Sign in"}</button>
            <button class="btn secondary" data-action="login-google">Google sign in</button>
            <button class="btn secondary" data-action="toggle-auth-mode">${state.authMode === "signup" ? "Use sign in" : "Need account?"}</button>
          </div>
...
async function handleAction(action, id) {
  ...
  if (action === "login-password") {
    if (!state.authEmail || !state.authPassword) return window.alert("Enter email and password.");
    const user = state.authMode === "signup"
      ? await window.RVUFirebase.createEmailPasswordAccount(state.authEmail, state.authPassword)
      : await window.RVUFirebase.signInWithEmailPassword(state.authEmail, state.authPassword);
    await enter(user);
    return;
  }
